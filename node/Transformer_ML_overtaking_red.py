...
size_of_input = 6
# 每个队列将存储最近的6个时间步的数据。这个值决定了模型在做预测时将考虑的时间窗口的大小
...
if __name__ == '__main__':
    ...
    # overtaking_model = load_model(rospack.get_path("f1tenth_simulator_two_agents")+'/overtaking_models/shanghai_Transformer')
    overtaking_model = load_model(rospack.get_path("f1tenth_simulator_two_agents")+'/overtaking_models/12_short_sequence_10_adjust_500')
    # overtaking_model = load_model(rospack.get_path("f1tenth_simulator_two_agents")+'/overtaking_models/model_RNN_10_4')
    ...
    while not rospy.is_shutdown():
        if LiDAR_raw_array.qsize() == size_of_input and car_state_array.qsize() == size_of_input:
